...
digitalWrite(AmpelRot, HIGH); //Dauerhaft Rot
servoMotor.write(5); //Schranke zu, kann nicht 95 sein sonst wechselt er von 95 zu 95
 if (Entfernung < 80) {
   trafficLightControl(0, 1, 0); //Wenn weniger als 80cm Abstand Gelb 1 Sekunde
   digitalWrite(AmpelGruen, HIGH); // Dann wird grün geschalten dauerhaft bis delay danach
   servoMotor.write(95); //Schranke öffnet
   delay(5000); /delay bevor rot geschalten wird
   digitalWrite(AmpelRot, HIGH); //wieder Dauerhaft Rot
  } 
} 
